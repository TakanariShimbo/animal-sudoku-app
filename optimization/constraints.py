...
def add_constraints(model: cp_model.CpModel, table: Table, variables: Variables) -> None:
    ...
    # -------------------------------------------------
    for number in TABLE_CONFIG.numbers:
        for h_grid_position in TABLE_CONFIG.h_grid_positions:
            for v_grid_position in TABLE_CONFIG.v_grid_positions:

                is_assigned_vars: list[cp_model.BoolVarT] = []
                for h_position_in_grid in TABLE_CONFIG.h_positions_in_grid:
                    for v_position_in_grid in TABLE_CONFIG.v_positions_in_grid:
